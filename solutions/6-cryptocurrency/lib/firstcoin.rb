...
every(3.seconds) do
  ...
  $PEERS.dup.each do |host|
    ...
    begin
      ...
      gossip_response = JSON.load(Client.gossip(host, $PEERS, blockchain_a))
...
post '/send_money' do
  ...
  is_from_node = request.ip === HOST && txn_h['from'] === MY_PUBLIC_KEY
  if is_from_node
    txn_h['priv_key'] = MY_PRIVATE_KEY
    txn_h['signature'] = nil
  end

  # TODO get tx, and instance it. If valid, mine it!
  txn =  Transaction.new(txn_h)
  if txn.is_valid_signature?
