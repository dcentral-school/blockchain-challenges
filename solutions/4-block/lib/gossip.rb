...
if PEER_HOST.nil?
  ...
  $BLOCKCHAIN = BlockChain.new(Message.new({'content' => @favorite_band, 'from' => MY_PUBLIC_KEY, 'priv_key' => MY_PRIVATE_KEY}))
else
...
every(8.seconds) do
  ...
  @favorite_band = BANDS.sample
  $BLOCKCHAIN.add_to_chain(Message.new({'content' => @favorite_band, 'from' => MY_PUBLIC_KEY, 'priv_key' => MY_PRIVATE_KEY}))
end
